...
trait LazyExtensionCompanion {
  ...
  def initImpl[T](c: whitebox.Context)(implicit tag: c.WeakTypeTag[T]): c.Tree = {
    import c.universe._
    import c.ImplicitCandidate

    val tpe = (c.openImplicits.headOption, tag.tpe.dealias) match {
      case (Some(ImplicitCandidate(_, _, tpe, _)), _) =>
        tpe.map(_.dealias)
      case (None, tpe) =>
        tpe
      case _ =>
    ...
    LazyMacros.dcRef match {
      case None =>
        val res = c.inferImplicitValue(appliedType(typeOf[Strict[_]].typeConstructor, tpe))
        if (res == EmptyTree)
