...
final class TupleOps[T](t: T) {
   ...
   */
  def +:[E](e: E)(implicit prepend: TuplePrepend[Tuple1[E], T]): prepend.Out = prepend(Tuple1(e), t)

  /**
   * Append the argument element to this tuple.
   */
  def :+[E](e: E)(implicit prepend: TuplePrepend[T, Tuple1[E]]): prepend.Out = prepend(t, Tuple1(e))

  /**
   * Append the argument tuple to this tuple.
   */
  def ++[U](u: U)(implicit prepend: TuplePrepend[T, U]): prepend.Out = prepend(t, u)
  
  /**
   * Prepend the argument tuple to this tuple.
   */
  def ++:[U](u: U)(implicit prepend: TuplePrepend[U, T]): prepend.Out = prepend(u, t)
   ...
   */
  def reverse_:::[U](u: U)(implicit prepend: TupleReversePrepend[U, T]): prepend.Out = prepend(u, t)
   ...
   */
  def init(implicit init: TupleInit[T]): init.Out = init(t)
   ...
   */
  def select[U](implicit selector: TupleSelector[T, U]): selector.Out = selector(t)

  /**
   * Returns all elements of type `U` of this tuple. An explicit type argument must be provided.
   */
  def filter[U](implicit filter: TupleFilter[T, U]): filter.Out = filter(t)

  /**
   * Returns all elements of type different than `U` of this tuple. An explicit type argument must be provided.
   */
  def filterNot[U](implicit filterNot: TupleFilterNot[T, U]): filterNot.Out = filterNot(t)
   ...
   */
  def removeElem[U](implicit remove: TupleRemove[T, U]): remove.Out = remove(t)
   ...
   * types in `S`.
   */
  def removeAll[S](implicit removeAll: TupleRemoveAll[T, S]): removeAll.Out = removeAll(t)
   ...
   * of type `U`.
   */
  def replace[U](u: U)(implicit replacer: TupleReplacer[T, U, U]): replacer.Out = replacer(t, u)
  
  class ReplaceTypeAux[U] {
    def apply[V](v: V)(implicit replacer: TupleReplacer[T, V, U]): replacer.Out = replacer(t, v)
  }
  ...
  def updatedElem[U, R](u: U)
    (implicit replacer: TupleReplacer.Aux[T, U, U, (U, R)]): R = replacer(t, u)._2
  
  class UpdatedTypeAux[U] {
    def apply[V, R](v: V)(implicit replacer: TupleReplacer.Aux[T, V, U, (U, R)]): R = replacer(t, v)._2
  }
  ...
  class UpdatedAtAux[N <: Nat] {
    def apply[U, V, R](u: U)(implicit replacer: TupleReplaceAt.Aux[T, N, U, (V, R)]): R = replacer(t, u)._2
  }
   ...
   * evidence that this tuple has at least ''n'' elements.
   */
  def updatedAt[U, V, R](n: Nat, u: U)(implicit replacer: TupleReplaceAt.Aux[T, n.N, U, (V, R)]): R = replacer(t, u)._2
   ...
   */
  def take[N <: Nat](implicit take: TupleTake[T, N]): take.Out = take(t)

  /**
   * Returns the first ''n'' elements of this tuple. Available only if there is evidence that this tuple has at
   * least ''n'' elements.
   */
  def take(n: Nat)(implicit take: TupleTake[T, n.N]): take.Out = take(t)
   ...
   */
  def drop[N <: Nat](implicit drop: TupleDrop[T, N]): drop.Out = drop(t)

  /**
   * Returns all but the  first ''n'' elements of this tuple. Available only if there is evidence that this tuple
   * has at least ''n'' elements.
   */
  def drop(n: Nat)(implicit drop: TupleDrop[T, n.N]): drop.Out = drop(t)
   ...
   */
  def split[N <: Nat](implicit split: TupleSplit[T, N]): split.Out = split(t)

  /**
   * Splits this tuple at the ''nth'' element, returning the prefix and suffix as a pair. Available only if there is
   * evidence that this tuple has at least ''n'' elements.
   */
  def split(n: Nat)(implicit split: TupleSplit[T, n.N]): split.Out = split(t)
   ...
   */
  def reverse_split[N <: Nat](implicit split: TupleReverseSplit[T, N]): split.Out = split(t)

  /**
   * Splits this tuple at the ''nth'' element, returning the reverse of the prefix and suffix as a pair. Available
   * only if there is evidence that this tuple has at least ''n'' elements.
   */
  def reverse_split(n: Nat)(implicit split: TupleReverseSplit[T, n.N]): split.Out = split(t)
   ...
   * of type `U`.
   */
  def splitLeft[U](implicit splitLeft: TupleSplitLeft[T, U]): splitLeft.Out = splitLeft(t)
   ...
   */
  def reverse_splitLeft[U](implicit splitLeft: TupleReverseSplitLeft[T, U]): splitLeft.Out = splitLeft(t)
   ...
   * of type `U`.
   */
  def splitRight[U](implicit splitRight: TupleSplitRight[T, U]): splitRight.Out = splitRight(t)
   ...
   * an element of type `U`.
   */
  def reverse_splitRight[U](implicit splitRight: TupleReverseSplitRight[T, U]): splitRight.Out = splitRight(t)

  /**
   * Reverses this tuple.
   */
  def reverse(implicit reverse: TupleReverse[T]): reverse.Out = reverse(t)

  /**
   * Maps a higher rank function across this tuple.
   */
  def map(f: Poly)(implicit mapper: TupleMapper[T, f.type]): mapper.Out = mapper(t)

  /**
   * Flatmaps a higher rank function across this tuple.
   */
  def flatMap(f: Poly)(implicit mapper: TupleFlatMapper[T, f.type]): mapper.Out = mapper(t)

  /**
   * Replaces each element of this tuple with a constant value.
   */
  def mapConst[C](c: C)(implicit mapper: TupleConstMapper[T, C]): mapper.Out = mapper(t, c)
   ...
   * type of ''op''.
   */
  def foldMap[R](z: R)(f: Poly)(op: (R, R) => R)(implicit folder: TupleMapFolder[T, R, f.type]): R = folder(t, z, op)
   ...
   */
  def foldLeft[R](z: R)(op: Poly)(implicit folder: TupleLeftFolder[T, R, op.type]): folder.Out = folder(t, z)
  
  /**
   * Computes a right fold over this tuple using the polymorphic binary combining operator `op`. Available only if
   * there is evidence `op` can consume/produce all the partial results of the appropriate types.
   */
  def foldRight[R](z: R)(op: Poly)(implicit folder: TupleRightFolder[T, R, op.type]): folder.Out = folder(t, z)
   ...
   */
  def reduceLeft(op: Poly)(implicit reducer: TupleLeftReducer[T, op.type]): reducer.Out = reducer(t)
   ...
   * results of the appropriate types.
   */
  def reduceRight(op: Poly)(implicit reducer: TupleRightReducer[T, op.type]): reducer.Out = reducer(t)

  /**
   * Zips this tuple with its argument tuple returning a tuple of pairs.
   */
  def zip[R](r: R)(implicit transpose: TupleTransposer[(T, R)]): transpose.Out = transpose((t, r))
   ...
   */
  def zipApply[A](a: A)(implicit zipper: TupleZipApply[T, A]): zipper.Out = zipper(t, a)
   ...
   */
  def zip(implicit transpose: TupleTransposer[T]): transpose.Out = transpose(t)

  /**
   * Unzips this tuple of tuples returning a tuple of tuples. Available only if there is evidence that this
   * tuple has tuple elements.
   */
  def unzip(implicit transpose: TupleTransposer[T]): transpose.Out = transpose(t)
   ...
   */
  def zipOne[R](r: R)(implicit zipOne: TupleZipOne[T, R]): zipOne.Out = zipOne(t, r)
  
  /**
   * Transposes this tuple.
   */
  def transpose(implicit transpose: TupleTransposer[T]): transpose.Out = transpose(t)

  /**
   * Returns a tuple typed as a repetition of the least upper bound of the types of the elements of this tuple.
   */
  def unify(implicit unifier : TupleUnifier[T]) : unifier.Out = unifier(t)

  /**
   * Returns a tuple with all elements that are subtypes of `B` typed as `B`.
   */
  def unifySubtypes[B](implicit subtypeUnifier : TupleSubtypeUnifier[T, B]) : subtypeUnifier.Out = subtypeUnifier(t)
  
  /**
   * Compute the length of this tuple.
   */
  def length(implicit length : TupleLength[T]) : length.Out = length(t)

  /**
   * Converts this tuple to a `List` of elements typed as the least upper bound of the types of the elements
   * of this tuple.
   */
  def toList[Lub](implicit toList : TupleToList[T, Lub]) : toList.Out = toList(t)
   ...
   */
  def toArray[Lub](implicit toArray : TupleToArray[T, Lub]) : toArray.Out = toArray(t)
}
