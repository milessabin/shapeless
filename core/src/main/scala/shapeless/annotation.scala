...
object AnnotationMacros {
  def mkAnnotation[A: Type, T: Type](given qctx: QuoteContext): Expr[Annotation[A, T]] = {
    ...
    if (annotFlags.is(Flags.Abstract) || annotFlags.is(Flags.Trait)) {
      qctx.error(s"Bad annotation type ${annotTpe.show} is abstract")
      '{???}
    } else {
      ...
      annoteeTpe.typeSymbol.annots.find(_.tpe <:< annotTpe) match {
        ...
        case None =>
          qctx.error(s"No Annotation of type ${annotTpe.show} for type ${annoteeTpe.show}")
          '{???}
      }
    }
  }
  ...
  def mkAnnotations[A: Type, T: Type](m: Expr[Mirror.Of[T]])(given qctx: QuoteContext): Expr[Annotations[A, T]] = {
    ...
    if (annotFlags.is(Flags.Abstract) || annotFlags.is(Flags.Trait)) {
      ...
      def mkAnnotations(annotTrees: Seq[Expr[Any]]): Expr[Annotations[A, T]] =
        annotTrees.toExprOfTuple match {
      ...
      annoteeTpe.classSymbol match {
        ...
        case Some(annoteeCls) =>
          Mirror(m) match {
            ...
            case None =>
              qctx.error(s"No Annotations for sum type ${annoteeTpe.show} with no Mirror")
              '{???}
          }
        case None =>
          qctx.error(s"No Annotations for non-class ${annoteeTpe.show}")
          '{???}
      }
    }
  }
