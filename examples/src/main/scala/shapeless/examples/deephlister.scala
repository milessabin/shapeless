...
trait LowPriorityDeepHLister {
  ...
  implicit def headNotCaseClassDeepHLister[H, T <: HList, OutT <: HList](implicit
    dht: Lazy[DeepHLister.Aux[T, OutT]]
  ): Aux[H :: T, H :: OutT] = new DeepHLister[H :: T] {
    type Out = H :: OutT
...
object DeepHLister extends LowPriorityDeepHLister {
  ...
  implicit def headCaseClassDeepHLister[H, R <: HList, T <: HList, OutR <: HList, OutT <: HList](implicit
    gen: Generic.Aux[H, R],
    dhh: Lazy[DeepHLister.Aux[R, OutR]],
    dht: Lazy[DeepHLister.Aux[T, OutT]]
  ): Aux[H :: T, OutR :: OutT] = new DeepHLister[H :: T] {
    type Out = OutR :: OutT
